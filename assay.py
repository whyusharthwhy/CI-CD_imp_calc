...
def table_extratcor_with_range(tables, headers, rt_range, software):
    """ Extracts tables with compounds that match a range from the PDFs using the camelot module"""
    rt_col = 'Ret. Time' if software == 'Lab Solutions' else 'RT'
    df_result_table =''
    result_tables = []
    for table in tables:
        df_table = table.df
        ...
        if(inx):
            ...
            df_table.columns = new_header
            df_table = df_table[headers]
            sample_rt = float(df_table[rt_col].values.tolist()[0])
...
def fill_assay_sheet(output_sheet, df_area_table, df_peak_table, sample_input_list, input_list):
    ...
    #title
    title = input_list[15] + " Assay calculation sheet"
    setOutCell(output_sheet, 1, 1, title)
    #poject name
...
#   Impurity table
    ...
    for index, row in df_peak_table.iterrows():
        if(table_row > 68):
            break
        # AR NO
        setOutCell(output_sheet, 1, table_row, row[10])
        # Batch NO
        ...
        #  Assay MG
        setOutCell(output_sheet, 13, table_row, row[11])
        table_row +=1
...
def initiate_report_creation(chrom_inputs, area_input, input_list, rt_range):
    ...
    if(rt_range):
        df_area_table = table_extratcor_with_range(tables, area_headers, rt_range, software)
    else:
    ...
    for index, chrom_input in enumerate(chrom_inputs):
        ...
        df_peak_table = df_peak_table[['B.no', 'Condition', 'Sample quantity', 'V1', 'V2', 'V3', 'Label claim', 'Per Unit', 'Area', 'Assay%', 'AR No.', 'Assay MG']]
